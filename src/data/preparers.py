...
class PropertyPredictionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Return features and a dictionary of targets for the given index
...
class DTIDatasetPreparer:
    ...
    def prepare_dti_dataset(
        ...
        """
        # Define a cache suffix based on parameters that affect processing
        ...
        try:
            ...
            # Featurize molecules and proteins
            logger.info(
            ...
            for smiles, protein_seq, affinity in zip(
                ...
                # Truncate protein sequence and prepare indices
                ...
                try:
                    ...
                    # Pad protein indices to ensure consistent length
            ...
            metadata = {
                ...
                "protein_max_length": max_prot_len,  # Use the defined max length
...
# Mapping of dataset names to their respective PyTDC classes for property prediction
"""
Data Preparation Module: Custom datasets and dataset preparers.
...
TDC_PROPERTY_DATA_MAPPING = {
    ...
    "PPB_Watanabe": ADME,
    "VDss_Lombardo": ADME,
    "Solubility_AqSolDB": ADME,  # Changed from Solubility to ADME
    # Add other property prediction datasets as needed
}
...
class PropertyDatasetPreparer:
    ...
    def __init__(self, featurizer: Optional[MolecularFeaturizer] = None):
        self.featurizer = featurizer or MolecularFeaturizer()
        # self.tdc_loader = TDCDataLoader() # Removed generic TDCDataLoader

        # Define min/max for normalization based on common ranges
        # These can be refined with actual dataset statistics
        self.logp_min, self.logp_max = (
            -3.0,
            5.0,
        )  # Typical range, will normalize to -1 to 1
        self.mw_min, self.mw_max = (
            50.0,
            800.0,
        )  # Common drug-like range, will normalize to 0 to 1
        self.tpsa_min, self.tpsa_max = (
            0.0,
            200.0,
        )  # Common TPSA range, will normalize to 0 to 1
    ...
    def prepare_property_dataset(
        self,
        dataset_name: str = "Lipophilicity_ID",  # Example dataset with LogP
        val_split: float = 0.1,
        test_split: float = 0.1,
        ...
        if DataCache.has_processed_data(
            ...
            try:
                ...
                if cached is not None:
                    ...
                    # Convert target lists back to dict of tensors if loaded as lists
        ...
        try:
            # Load data from TDC. We assume it provides 'smiles' and potentially 'y' (e.g., LogP)
            # For properties like QED, SA, MW, we calculate them
            ...
            # Initialize lists for calculated properties and features
            all_features = []
            ...
            for smiles in smiles_list:
                ...
                tpsa = Descriptors.TPSA(mol)  # Using TPSA as a proxy for SA
                ...
                all_features.append(
                    torch.tensor(mol_features).squeeze(0)
                )  # Convert to tensor
                all_qed.append(qed)
                all_sa.append(self._normalize_tpsa(tpsa))  # Normalize TPSA
                all_logp.append(self._normalize_logp(logp))  # Normalize LogP
                all_mw.append(self._normalize_mw(mw))  # Normalize MW
            ...
            metadata = {
                ...
                "qed_min_max": (0.0, 1.0),  # QED is inherently 0-1
            ...
            # Save to cache - targets need to be lists for pickling or similar, not dict of tensors directly
            # Convert targets_dict to list of lists/arrays for caching
